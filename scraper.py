...
with NO LIMIT and pushes them to Firebase Firestore every 5 seconds.
"""
...
class ParimatchCricketLiveScraper:
    ...
    def __init__(
        self,
        cert_path: str = "serviceAccountKey.json",
        interval: float = 5.0,
        headless: bool = True
    ):
        ...
        # Initialize Firebase
        print(f"  [Firebase] Initializing with key: {cert_path}...", flush=True)
        try:
            if not firebase_admin._apps:
                cred = credentials.Certificate(cert_path)
                firebase_admin.initialize_app(cred)
            self.db = firestore.client()
            print("  [Firebase] Successfully connected to Firestore.", flush=True)
        except Exception as e:
            print(f"  [Firebase] Error initializing Firebase: {e}", flush=True)
            print("  [Firebase] Make sure 'serviceAccountKey.json' exists in this directory.", flush=True)
            self.db = None

    def push_to_firebase(self, payload: Dict[str, Any]) -> None:
        if not self.db:
    ...
    async def run(self, once: bool = False) -> None:
        """Main real-time loop."""
        print("=" * 70)
        print("CRICKET ODDS LIVE SCRAPER | FIREBASE REAL-TIME SYNC")
        print(f"Interval: {self.interval}s  |  Mode: {'Single-shot' if once else 'Continuous 5s Feed'}")
        ...
        async with async_playwright() as p:
            ...
            while self.running:
                ...
                try:
                    ...
                    # 4. Push payload to Firebase
                    payload = self.build_payload(iteration, sorted_matches)
                    self.push_to_firebase(payload)
                    ...
                    print(
                        ...
                        f"{elapsed*1000:.1f}ms -> Firebase Firestore",
                        flush=True
                    )
...
def main():
    parser = argparse.ArgumentParser(description="Parimatch Cricket Live Odds Scraper to Firebase")
    parser.add_argument("--cert", "-c", default="serviceAccountKey.json", help="Path to Firebase Service Account JSON file")
